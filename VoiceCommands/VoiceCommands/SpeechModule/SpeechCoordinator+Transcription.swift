...
extension SpeechCoordinator {
    ...
    func transcribeAudio() throws {
        ...
        let contextualString = commandsToRecognise.map{ $0.commandString }
        ...
        recognitionTask = speechRecognizer.recognitionTask(with: recognitionRequest) { [weak self] result, error in
            ...
            if error != nil || isFinal {
                ...
                debugPrint("\(SpeechCoordinator.identifier) transcribeAudio \(DebuggingIdentifiers.actionOrEventFailed) Audio Transcription Failed with error :\(String(describing: error?.localizedDescription)).")
            }
        }
